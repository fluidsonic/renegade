...
MovieOptionsMenuClass::On_Init_Dialog (void)
{
	...
	if (list_ctrl != NULL) {
		...
		if (registry.Is_Valid ()) {
			...
			if (item_index != -1) {
				list_ctrl->Set_Entry_Data (item_index, 0, (DWORD)(uintptr_t)new StringClass (INTRO_MOVIE));
			}
			...
			for (int index = 0; index < list.Count (); index ++) {
				...
				if (item_index != -1) {
					list_ctrl->Set_Entry_Data (item_index, 0, (DWORD)(uintptr_t)new StringClass (list[index]));
				}
			}
		}
	}
