 ...
 *=============================================================================================*/
ServerSettingsClass::ServerSettingsClass(char *filename, const WCHAR *configname, int file_number)
{
	...
	*/
	...
	for (int i=2 ; i<MAX_SETTINGS_FILES ; i++) {
		...
		if (file.Is_Available()) {
			...
			if (ini) {
				description = ini->Get_Wide_String(description, "Settings", "wConfigName", u"");
				if (description.Get_Length()) {
					...
					WideStringClass tmp(char_description, true);
					ServerSettingsList.Add(new ServerSettingsClass(file_name, tmp.Peek_Buffer(), i));
				}
			}
		}
	}
