...
const WideStringClass& INIClass::Get_Wide_String(WideStringClass& new_string, char const * section, char const * entry, WCHAR const * defvalue) const
{
	...
	if (length == 0) {
		new_string = (const WCHAR*)defvalue;
	} else {
		int outcount = b64pipe.Put(buffer, length);
		outcount += b64pipe.End();
		new_string = (const WCHAR*)out;
	}
	return(new_string);
}
...
bool INIClass::Put_Wide_String(char const * section, char const * entry, const WCHAR * string)
{
	...
	if (len == 0) {
		...
		BufferStraw straw(string, (len * (int)sizeof(WCHAR)) + (int)sizeof(WCHAR));	// Convert from WCHARs to bytes, plus terminator.
		Base64Straw bstraw(Base64Straw::ENCODE);
		bstraw.Get_From(straw);
