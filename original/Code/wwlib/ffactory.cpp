...
{
	int sub_len = static_cast<int32_t>(strlen(sub_directory));
	// Overflow prevention
	if (sub_len > 1021) {
		return;
	} else if (sub_len < 1) {
...
FileClass * SimpleFileFactoryClass::Get_File( char const *filename )
{
	...
	if (IsStripPath) {
		const char* lastFwd = ::strrchr(filename, '/');
		const char* lastBack = ::strrchr(filename, '\\');
		const char* ptr = (lastBack > lastFwd) ? lastBack : lastFwd;
