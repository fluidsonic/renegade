	...
	unsigned short vertex_count)
{
	...
	D3DXMATRIX mtx;
	D3DXMatrixMultiply(&mtx,(const D3DXMATRIX*)&state->sorting_state.world,(const D3DXMATRIX*)&state->sorting_state.view);
...
void SortingRendererClass::Flush_Sorting_Pool()
{
	...
	DynamicVBAccessClass dyn_vb_access(BUFFER_TYPE_DYNAMIC_DX8,dynamic_fvf_type,overlapping_vertex_count);
	{
		...
		for (node_id=0;node_id<overlapping_node_count;++node_id) {
			...
			D3DXMATRIX d3d_mtx;
			D3DXMatrixMultiply(&d3d_mtx,(const D3DXMATRIX*)&state->sorting_state.world,(const D3DXMATRIX*)&state->sorting_state.view);
