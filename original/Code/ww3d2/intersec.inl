...
#define AXIS_3 2
	...
	if(intersect) {
		...
		Point[AXIS_3] = static_cast<float>(u3 * alpha + v3 * beta) + Corner1[AXIS_3];
	}
	...
	float &Beta) 
{
	...
	if (u1 == 0.0f)    {
	  Beta = static_cast<float>(beta = u0 / u2);			// beta is the percentage down the edge loc1->loc3
	  if ((beta >= 0.0) && (beta <= 1.0)) {		// make sure it's within the edge segment
			Alpha = static_cast<float>(alpha = (v0 - beta * v2) / v1);	 // alpha is the percentage down the edge loc1->loc2
	  ...
	  Beta = static_cast<float>(beta = (v0 * u1 - u0 * v1) / (v2 * u1 - u2 * v1));
	  if ((beta >= 0.0) && (beta <= 1.0)) {
			Alpha = static_cast<float>(alpha = (u0 - beta * u2) / u1);
			intersect = ((alpha >= 0.0) && ((alpha + beta) <= 1.0));
	  }
	}
...
inline float IntersectionClass::Plane_Z_Distance(Vector3 &PlaneNormal, Vector3 &PlanePoint) 
{
	...
	float value = static_cast<float>(- (d + PlaneNormal[0] * (*RayLocation)[0] + PlaneNormal[1] *  (*RayLocation)[1] + PlaneNormal[2] * (*RayLocation)[2]) / divisor);
	...
	Vector3 &PlaneNormal)
{
	...
	float value = static_cast<float>(- (d + PlaneNormal[0] * Point[0] + PlaneNormal[1] *  Point[1] ) / PlaneNormal[2]);
