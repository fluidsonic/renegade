...
int* StripOptimizerClass::Combine_Strips (const int* strips, int strip_count)
{
	...
	delete[] tmp;
//	assert(check == (o-out-1));
	*out = static_cast<int32_t>(o-out-1);						// set length
	return out;
}
...
struct Triangle
{
	...
	const Edge	getEdge			(int i) const	{ assert(i>=0 && i<3); return Edge(m_vertices[i],i==2?m_vertices[0]:m_vertices[i+1]); }
...
private:
	...
	static int			getMod3								(int i) { assert(i>=0 && i<6); return s_mod[i]; }
...
//	printf ("total swaps   = %d\n",nSwaps);
	...
	int32_t	len		= static_cast<int32_t>(o-out);											// allocation length
