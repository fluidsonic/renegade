...
HeightDBClass::Get_Height (const Vector3 &pos)
{
	...
	if (m_HeightArray != NULL && m_NumPointsX > 0 && m_NumPointsY > 0) {
		...
		if (percent_x >= 0 && percent_x <= 1.0F && percent_y >= 0 && percent_y <= 1.0F) {
			...
			int entry_ul_x = static_cast<int32_t>((m_NumPointsX - 1) * percent_x);
			int entry_ul_y = static_cast<int32_t>((m_NumPointsY - 1) * percent_y);
...
HeightDBClass::Generate (void)
{
	...
	//
	m_NumPointsX	= static_cast<int32_t>(((m_LevelMax.X - m_LevelMin.X) / generate_patch_size) + 1);
	m_NumPointsY	= static_cast<int32_t>(((m_LevelMax.Y - m_LevelMin.Y) / generate_patch_size) + 1);
