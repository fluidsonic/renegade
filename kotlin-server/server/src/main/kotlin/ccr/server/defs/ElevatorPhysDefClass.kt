...
class ElevatorPhysDefClass(
    ...
    val lowerCallZone: ElevatorOBBox = ElevatorOBBox(),
    val lowerInsideZone: ElevatorOBBox = ElevatorOBBox(),
    val upperCallZone: ElevatorOBBox = ElevatorOBBox(),
    val upperInsideZone: ElevatorOBBox = ElevatorOBBox(),
    ...
    companion object {
        ...
        fun load(
            ...
            var closeDelay                   = 2f
            var lowerCallZone                = ElevatorOBBox()
            var lowerInsideZone              = ElevatorOBBox()
            var upperCallZone                = ElevatorOBBox()
            var upperInsideZone              = ElevatorOBBox()
            ...
            vars.forEachMicroChunk { mcId, data ->
                when (mcId) {
                    ...
                    MCID_UPPER_CALL_ZONE          -> upperCallZone = parseElevatorOBBox(data)
                    MCID_UPPER_INSIDE_ZONE        -> upperInsideZone = parseElevatorOBBox(data)
                    MCID_LOWER_CALL_ZONE          -> lowerCallZone = parseElevatorOBBox(data)
                    MCID_LOWER_INSIDE_ZONE        -> lowerInsideZone = parseElevatorOBBox(data)
...
/**
 * Oriented bounding box used by ElevatorPhysDefClass.
 * Center(3) + Extent(3) + Basis(3x3 rotation matrix).
 */
data class ElevatorOBBox(
...
private fun parseElevatorOBBox(data: ByteArray): ElevatorOBBox {
    if (data.size < 24) return ElevatorOBBox()
    ...
    return if (data.size >= 60) {
        ElevatorOBBox(centerX, centerY, centerZ, extentX, extentY, extentZ,
